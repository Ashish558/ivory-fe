            ...
            <div className='flex flex-col gap-y-2 justify-around items-center' >
               {options.map(option => {
                  return <div key={option.id}
                     className={`w-full bg-[#0055BF] text-white py-1 flex justify-center items-center font-bold rounded-full cursor-pointer ${option.selected ? `${styles.selectedOption}` : ''} ${mcqResponse.selected === true && option.is_correct === true ? `${styles.correctOption}` : ''}  ${mcqResponse.selected === true && option.is_correct === false && option.selected ? `${styles.incorrectOption}` : ``}`}
